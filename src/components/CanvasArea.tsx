  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (tool === "eraser") {
      ...
      targetLayers.forEach((lay) => {
        ...
        let spacingValue = useStore.getState().brushSpacing;
        if (['spray', 'charcoal', 'chalk', 'pastel', 'sponge', 'splatter', 'stipple', 'screentone', 'screentone-manga', 'halftone'].includes(brushTexture)) {
          spacingValue = Math.max(spacingValue, 45);
        } else if (['pencil', 'crayon', 'oil', 'dry-brush', 'canvas-grain'].includes(brushTexture)) {
          spacingValue = Math.max(spacingValue, 25);
        }
